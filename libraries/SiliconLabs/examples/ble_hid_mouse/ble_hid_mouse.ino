/*
   ...
   The example implements a simple BLE HID mouse device that has 1 mouse button and uses the
   IMU (Inertial Measurement Unit) acceleration data to move the cursor.

   Find out more on the API usage at: https://docs.silabs.com/bluetooth/latest/bluetooth-stack-api/

   Compatible boards:
   - xG24 DevKit (ICM20689)
   - xG27 DevKit (ICM20689)

   Libs:
   - Silicon Labs package (v2.0.0)
   - ICM20689 (v1.0.0) by Inhwan Wee library

   Author: Zoltan Fegyveres (Silicon Labs)
 */
#include "ICM20689.h"

#define DEVICE_NAME "Arduino HID Mouse"
...
#define IMU_CIPO PC2
#define IMU_SCLK PC1
#define IMU_INT  PB1 // For future use
...
#define IMU_GYRO_X_THRESHOLD 2
#define IMU_GYRO_Y_THRESHOLD 2
#define IMU_GYRO_Z_THRESHOLD 2
...
void loop()
{
  ...
  if (IMU.readSensor() < 0) {
    ...
    acc_y = (int32_t)IMU.getAccelX_mss();
    acc_x = (int32_t)IMU.getAccelY_mss();
