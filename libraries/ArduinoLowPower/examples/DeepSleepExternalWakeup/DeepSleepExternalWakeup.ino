/*
   ArduinoLowPower deep sleep example with external wakeup

   The example shows the basic usage of the Arduino Low Power library by putting the device into deep sleep.
   The device will remain in deep sleep until an external interrupt is triggered.
...
void setup()
{
  ...
  Serial.println("Deep sleep with external wakeup");
...
void loop()
{
  ...
  Serial.printf("Going to deep sleep at %lu\n", millis());
  LowPower.deepSleep();
}
